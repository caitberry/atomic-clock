...
# --- 1. Compile and Run ---
# Point to the new Stan file that estimates clock-specific dark uncertainties
fit <- stan(
...
# Extract clock-specific dark uncertainties (sigma_dark)
# apply() is used to calculate quantiles across the iterations (rows) for each clock (columns)
dark_unc_estimates <- colMeans(posterior_samples$sigma_dark)
...
# nu_0 contains all 14 reference frequencies parsed from 's' lines 
...
# We now add the clock-specific dark uncertainties directly into the results dataframe
...
cat("\n### Bayesian Optimized Frequencies & Clock-Specific Dark Uncertainties ###\n")
print(results_df)
