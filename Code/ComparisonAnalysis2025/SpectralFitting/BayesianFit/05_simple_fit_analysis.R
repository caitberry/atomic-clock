...
for(i in 1:length(unique(spectralEstDF$Date))){
  ...
  simpleFitRes=data.frame(h0=h0_est, CIlower=h0_ci[1],CIupper=h0_ci[2])
  ...
  p1 <- ggplot() +
    ...
    geom_segment(aes(x=min(fit_data$freq), xend=F_MAX_BAND,
                     y=exp(beta0_corrected), yend=exp(beta0_corrected)), color="red", size=1.5) +
    scale_x_log10() + scale_y_log10() +
    labs(x="Frequency", y="PSD") + theme_bw()
  
  plot1_filename <- paste0(Output_SimpleFit_Dir, "SimpleFit_", dataName, "_", gsub("-", "_", d), ".png")
  ggsave(plot1_filename, plot = p1, width = 5, height = 4)
  ...
  p2 <- ggplot(sensitivity_df, aes(x=cutoff, y=h0)) +
    ...
    scale_y_log10() +
    labs(x="Upper Cutoff", y="Estimated h0") + theme_bw()
  plot2_filename <- paste0(Output_SimpleFit_Dir, "SimpleFit_Sensitivity_", dataName, "_", gsub("-", "_", d), ".png")
  ggsave(plot2_filename, plot = p2, width = 5, height = 4)
  
  # Save Fit Object
  save_name <- paste0(Output_SimpleFit_Dir, "fit_",dataName,"_Date_", gsub("-", "_", d), ".rds")
  saveRDS(list(simplefit=simpleFitRes,sensitivity_df=sensitivity_df), file = save_name)
