...
x.t=formattedData$x.t[1:500]   ####################CHANGE THIS BACK
t.vec=formattedData$t.vec[1:500]####################CHANGE THIS BACK
N=length(x.t)#formattedData$N####################CHANGE THIS BACK

plot(t.vec,x.t)


write.csv(data.frame(x.t=x.t,t.vec=t.vec),file = "/home/aak3/NIST/atomic-clock/DataForVK_2024-04-24-ErYb-AlYb.csv",row.names = F)
test=read.csv("DataForVK_2024-04-24-ErYb-AlYb.csv")
head(test)
########################################################################################
...
source(file = paste(folderLocation,"Code/analysisFunctions.R",sep="")) ####################CHANGE THIS BACK (running again here for code testing)
myDatSpecEst=mtse$spectralEstWithUnc(x.t = x.t,t.vec=t.vec,N.fourier = floor(N/2) + 1,
                       numTapers = myK,calcCov = T,
                       myW = myW,isWhite = F,acf.lag = 4,numCores=14)
...
# write.csv(data.frame(x.t=x.t,t.vec=t.vec),file = "/home/aak3/NIST/atomic-clock/DataForVK_2024-04-24-ErYb-YbSr.csv",row.names = F)
# test=read.csv("DataForVK_2024-04-24-ErYb-YbSr.csv")
# head(test)
########################################################################################
# 3. get spectral estimate
# 3a) try a variety of W and K values and check eigenvalues to make sure you've made good selections
