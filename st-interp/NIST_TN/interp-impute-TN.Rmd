...
To derive a ratio time series for any two clocks, the time points of observations for each clock must be in alignment with those of the comb. There are several methodological approaches one may take to align all three series. 

**Method 1**: Align clock A with the observations of the comb. Then align clock B with the observations of the comb. Proceed to derive the ratio time series for analysis. The result of this approach will be a time series with the same irregularities in sampling as the comb series. [Add text on the pros and cons of this approach] 

**Method 2**: Align clock A with the observations of clock B. Then align both clock A and clock B with the comb. This approach may be considered if clock A has a much lower frequency of observations than clock B. As with Method 1, the result will be a time series with the same irregular observation pattern as the comb data. [Add text on the pros and cons of this approach]

**Method 3**: Realign the comb time series so that the observations are regularly sampled. Align clock A with the new, regularly observed comb series. Align clock B with the new, regularly observed comb series. [Add text on the pros and cons of this approach] 

**Method 4**: Align clock A with the observations of clock B. Then align the comb with clock B. The approach essentially disregards or masks comb data for the sake of matching the observational time points of clock B. One challenge of this approach is the question of how to summarize the comb data to match up with time points of clock B. [Add text on the pros and cons of this approach]
