...
[Background on existing methods for univariate time series imputation] @howe2021, @pavia2010, @knotters2010, @eckner2012

In  addition to providing a useful overview of common time series interpolation techniques, @lepot2017 also remark upon the assessment of uncertaintites that result from interpolation, noting that the law of propagation of uncertaintites does not apply in this case. 
...
The treatment of long sequences of missing data is distinct from the treatment of individual missing data points. While common imputation techniques for univariate time series can be implemented for individual missing values, long sequences of missing values indicate a need to reconsider the start and end times for each sub-series. Discussed in more detail later, interpolation techniques may be applied to impute individual missing data values. The only distinction between imputation and interpolation in this context is a matter of perspective as @nieto2015 remark that "unevenly spaced time series have also been treated as equally spaced time series with missing observations."

On the other hand, large gaps of missing data must be factored in to the next step where one decides which window of observations to analyze in the target series. This is the topic of the next section.   
