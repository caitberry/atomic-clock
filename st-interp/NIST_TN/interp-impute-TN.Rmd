...
For example, suppose the $Al+$ clock data is stored in a data frame called shift_data_Al. This data frame contains three variables, $t_{MJD}$, $f_{shift}$, and $IS_GOOD$. In order to analyze only the data that is marked as good by the clock scientist, we can use the following Python code to create a subset of only the good data, named shift_data_Al_good. 
