...
## Exclude Bad Clock Data 

Filter out all "not good" data. will probably not be any missing values after that but the next subsection addresses the case in which there are still missing values in the clock data. 

For example, suppose the $Al+$ clock data is stored in a data frame called shift_data_Al. This data frame contains three variables, $t_{MJD}$, $f_{shift}$, and $IS_{GOOD}$. In order to analyze only the data that is marked as good by the clock scientist, we can use the following Python code to create a subset of only the good data, named shift_data_Al_good. 
...
find overlapping MJD values for each clock frequency data set. decide upon initial and final MJD values for analysis 
